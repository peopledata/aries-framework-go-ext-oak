...
.PHONY: generate-vdr-oak-test-keys
generate-vdr-oak-test-keys:
	@rm -Rf ./test/bdd/vdr/oak/fixtures/keys/tls
	@mkdir -p -p test/bdd/vdr/oak/fixtures/keys/tls
	@docker run -i --rm \
		-v $(abspath .):/opt/workspace/ext \
		--entrypoint "/opt/workspace/ext/test/bdd/vdr/oak/generate_test_keys.sh" \
		frapsoft/openssl
